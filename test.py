...
import random
valid_chars = "0123456789_abcdefghijklmnopqrstuvwxyz_ABCDEFGHIJKLMNOPQRSTUVWXYZ"
...
class Bank:
    ...
    @classmethod
    def Loanability(self) -> bool: return self.__Can_Loan
    @classmethod
    def Users(self) -> dict: return self.__users

    @classmethod
    def update_Balance(self, Amount):
        self.__Bank_Balance += Amount
    @classmethod
    def update_Loaned(self, Amount):
        self.__Loan_Given += Amount
    @classmethod
    def update_Loanability(self, New):
        self.__Can_Loan = New

    def generate_acNumber(self, username) -> str:
        while(True):
            acNumber = username
            for c in range(5):
                acNumber += random.choice(valid_chars)
            if acNumber not in self.__users: return acNumber

    def create_account(self, acNumber, info):
        self.__users[acNumber] = info

    def delete_account(self, caller, acNumber):
        if (caller in self.__users) and (self.__users[caller][2]=='admin'):
            del self.__users[acNumber]
            print(f"Account no. {acNumber} is Deleted successfully")
        else: print("Request Denied! You don't have this access")
...
class User:
    ...
    def Withdraw(self, amount):
        if(self.__balance < amount):
            ...
            if(Bank.Balance() > 0):
                limit = min(Bank.Balance(), self.__balance)
                print(f"Your current withdrawal limit: {limit}/-")
        else:
            self.__balance -= amount
            Bank.update_Balance(-1 * amount)
            self.transaction('-', amount, 'Withdrawn')
            print(f"{amount}/- Withdrawn Successfully!")
            self.show_Balance()

    def take_loan(self):
        if(Bank.Loanability() is True) and (Bank.Balance() > 0):
            if(self.__loaned_time < 2):
                amount = int(input("Enter loan amount: "))
                if(Bank.Balance() < amount):
                    print(f"Sorry, current laon limit is {Bank.Balance()}")
                else:
                    self.Deposit(amount)
                    self.__loaned_time += 1
                    self.__loaned_amount += amount
                    Bank.update_Loaned(amount)
                    Bank.update_Balance(-1 * amount)
    ...
    def transfer(self, acNumber, amount):
        reciever = Bank.Users()[acNumber][0]
...
class Admin:
    ...
    def show_all__users(self):
        for (key, val) in Bank.Users().items():
            print(val[0])

    def delete_user(self, acNumber):
        Bank.delete_account(self.__acNumber, acNumber)
        # remove (balance-loaned) from bank account
        # there are some problems in passing self.__acNumber

    @property
    def check_balance(self):
        print("Current Bank Balance:", Bank.Balance())

    @property
    def check_loaned(self):
        print("Total Loans Given:", Bank.Loaned())

    @property
    def toggle_loan(self):
        Bank.update_Loanability(not Bank.Loanability())
        if(Bank.Loanability() is True): print("Loans Turned ON: users can take loans now")
...
while(True):
    ...
    if(cmd=='1'):
        ...
        acNumber = op.generate_acNumber(email.split("@")[0][:5])
    ...
    while(signed == 'user'):
        ...
        if c == '1':
            ...
            elif op.Users()[acNumber][2] == 'admin':
